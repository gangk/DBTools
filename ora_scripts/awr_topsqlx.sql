...
col module      format a8               heading "Module"
     ...
     select *
       from (
             select
                  ...
                  , nvl(substr(st.sql_text,1,6), to_clob('** SQL Text Not Available **')) sql_text     -- PUT/REMOVE COMMENT TO HIDE/SHOW THE SQL_TEXT
             from        (
                          select snap_id, tm, inst, dur, sql_id, phv, module, elap, elapexec, cput, iowait, appwait, concurwait, clwait, bget, dskr, dpath, rowp, exec, prsc, pxexec, aas, time_rank
                          from
                                             (
                                               select
                                                      s0.snap_id snap_id,
