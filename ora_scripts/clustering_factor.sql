...
&if_lower_v10 replace(replace(replace(replace(replace('&col_list', ' asc', ''), '
...
select        
case        
when '&p_degree' != 'DEFAULT' then 'parallel (t &p_degree)' else ''
end as p_degree_hint_expr
from        
dual;
-- set echo on verify on
set termout on
select sys_op_countchg(substrb(row_id,1,15), &history) 
