start	snap-rpt140	'Table Info'

--  SCRIPT:	table-info.sql
--  #DESC:	everything I need to know about a specific table and its indexes
...
col adbpk 			head "AvgData|Block|PerKey" format  999990
...
SELECT 	 table_name
	...
	,chain_cnt*100/decode(num_rows,0,999999999,num_rows) pctch
 	,avg_row_len
FROM   	 dba_tables 					t
WHERE 	 t.owner = upper(nvl('&&Owner',user)) 
...
SELECT 	 segment_name
	...
	,s.header_block					hdrb
FROM 	 dba_segments s
WHERE 	 s.owner = upper(nvl('&&Owner',user)) 
AND 	 segment_name = upper('&&Table_name') 
;

prompt ;
prompt ;
prompt -- SEGMENT DATAFILE --;

col owner	format a7
col seg		format a30	head 'Segment Name'
col segt	format a3	head 'Typ'	trunc
col file_mb	format 999999	head 'Size|MB'
col fid    	format 999	trunc
col fname	format a54	head 'File Name'
compute sum of file_mb on seg
compute sum of ext on seg
break on owner on seg on segt on tsname 

SELECT	 /*+ RULE */
	 segment_name 		seg
	,segment_type 		segt
	,e.tablespace_name 	tsname
	,sum(e.bytes)/(1024*1024)	file_mb
	,count(*)		ext
	,e.file_id		fid
	,df.file_name		fname
...
WHERE    b.constraint_type	= 'R'
  ...
  AND	 a.owner		= upper(nvl('&&Owner',user)) 
...
WHERE    fk.r_constraint_name 	= pk.constraint_name 
  ...
  AND	 fk.owner		= upper(nvl('&&Owner',user)) 
