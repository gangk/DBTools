...
in ('__shared_pool_size','shared_pool_reserved_size','__streams_pool_size','__db_cache_size',
    '__java_pool_size','result_cache_max_size','db_recycle_cache_size','db_keep_cache_size','db_2k_cache_size',
    'db_4k_cache_size','db_8k_cache_size','log_buffer','db_32k_cache_size','__large_pool_size',
    'pga_aggregate_target','sga_max_size','sga_target','__shared_io_pool_size',
    'shared_pool_size','large_pool_size','java_pool_size','streams_pool_size','db_cache_size',
    '__sga_target','memory_target','memory_max_target','__pga_aggregate_target')
...
from dba_hist_snapshot b, dba_hist_parameter a where parameter_name
in ('open_cursors','processes','sessions','db_files','shared_server',
    'session_cached_cursors','result_cache_max_result')
and a.snap_id = b.snap_id
and to_char(end_interval_time, 'YYYY dd-Mon HH24') = '&&endd'
order by 1,2
/
...
in ('cursor_sharing','query_rewrite_enabled','statistics_level', 'cursor_space_for_time',
     'db_cache_advice','compatible','event', 'result_cache_mode','pre_page_sga')
and a.snap_id = b.snap_id
and to_char(end_interval_time, 'YYYY dd-Mon HH24') = '&&endd'
order by 1,2
/
...
select  a.ksppinm  "Parameter",
     ...
     where a.indx = b.indx and a.indx = c.indx
       and a.ksppinm in ('_kghdsidx_count','__shared_pool_size','__streams_pool_size',
         ...
         '_kglsim_maxmem_percent','_shrunk_aggs_enabled',
...
col cursor_type head "Cursor Type"
col "Mbytes per Type" 999,999 head "MBytes per Type"


REM 
REM  for 11.2 and higher

select c.CURSOR_TYPE, sum(a.sharable_mem)/1024/1024 as "Mbytes per Type" 
from   V$OPEN_CURSOR c, v$sql a
where  c.hash_value=a.hash_value
and    c.CURSOR_TYPE='SESSION CURSOR CACHED'
group by c.CURSOR_TYPE
/
