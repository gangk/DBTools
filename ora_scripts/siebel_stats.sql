...
CL TIMI;
REM
REM $Header: coe_siebel_stats.sql 11.2 2010/11/19 csierra $
REM
...
CREATE OR REPLACE PACKAGE BODY siebel_stats AS
  ...
  IS
    ...
    l_exec VARCHAR2(12);
  BEGIN
    -- validates this is 10g or 11g, else EXIT
    DECLARE
      dot1 NUMBER;
      dot2 NUMBER;
    BEGIN
      SELECT version INTO l_rdbms_version FROM sys.v$instance;
      dot1 := INSTR(l_rdbms_version, '.');
      dot2 := INSTR(l_rdbms_version, '.', dot1 + 1);
      l_rdbms_release :=
      TO_NUMBER(SUBSTR(l_rdbms_version, 1, dot1 - 1)) +
      (TO_NUMBER(SUBSTR(l_rdbms_version, dot1 + 1, dot2 - dot1 - 1)) / POWER(10, (dot2 - dot1 - 1)));
      IF l_rdbms_release < 10 THEN
        RAISE_APPLICATION_ERROR(-20200, 'Use on 10g or higher, not in '||l_rdbms_version);
      END IF;
    END;
    ...
    DECLARE
      cnt NUMBER;
    BEGIN
      l_schema_owner := UPPER(NVL(TRIM(' ' FROM p_schema_owner), '&&default_schema_owner.'));
      SELECT COUNT(*) INTO cnt FROM sys.dba_users WHERE username = l_schema_owner;
      IF cnt != 1 THEN
        RAISE_APPLICATION_ERROR(-20300, 'Invalid schame owner: '||l_schema_owner);
      END IF;
    END;
