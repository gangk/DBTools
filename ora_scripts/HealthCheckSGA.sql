...
REM 
REM  runs on 9.2.x and 10g releases
REM
...
from dba_hist_snapshot b, dba_hist_parameter a where parameter_name
in ('cursor_sharing','query_rewrite_enabled','statistics_level','cursor_space_for_time',
     'db_cache_advice','compatible','event')
and a.snap_id = b.snap_id
and to_char(end_interval_time, 'YYYY dd-Mon HH24') = '&&endd'
order by 1,2
/
...
PROMPT  ##########  Non-Shared Code Should Not Exceed 20% Ideally  ##########  
PROMPT
set feed off
...
declare
   ...
   cursor c3 is select count(1), sum(decode(executions,1,1,0)),
      round(sum((sharable_mem+persistent_mem+runtime_mem))/1024/1024,0), 
      round(sum(decode(executions, 1, (sharable_mem+persistent_mem+runtime_mem)/1024/1024)),0)
...
begin 
   ...
   close c3;
     dbms_output.put_line(' ');
     dbms_output.put_line('FINDINGS: ### Ideal to keep memory for non-shared code < 20% .... Actual here: '||round(100*(EX1_MEMORY / TTLBYTES), 0)||'% ###');
end;
/
...
REM  Shows data in MBytes
REM

set feed on
PROMPT
PROMPT  ########## Historic V$SGASTAT Data in MBytes Leading to ORA-4031 Errors ########## 
set feed off

set lines 290

select n, 
   max(decode(to_char(begin_interval_time, 'hh24'), 1,to_char(bytes/1024/1024,'999,999'), null)) "1",
   max(decode(to_char(begin_interval_time, 'hh24'), 2,to_char(bytes/1024/1024,'999,999'), null)) "2",
   max(decode(to_char(begin_interval_time, 'hh24'), 3,to_char(bytes/1024/1024,'999,999'), null)) "3",
   max(decode(to_char(begin_interval_time, 'hh24'), 4,to_char(bytes/1024/1024,'999,999'), null)) "4",
   max(decode(to_char(begin_interval_time, 'hh24'), 5,to_char(bytes/1024/1024,'999,999'), null)) "5",
   max(decode(to_char(begin_interval_time, 'hh24'), 6,to_char(bytes/1024/1024,'999,999'), null)) "6",
   max(decode(to_char(begin_interval_time, 'hh24'), 7,to_char(bytes/1024/1024,'999,999'), null)) "7",
   max(decode(to_char(begin_interval_time, 'hh24'), 8,to_char(bytes/1024/1024,'999,999'), null)) "8",
   max(decode(to_char(begin_interval_time, 'hh24'), 9,to_char(bytes/1024/1024,'999,999'), null)) "9",
   max(decode(to_char(begin_interval_time, 'hh24'), 10,to_char(bytes/1024/1024,'999,999'), null)) "10",
   max(decode(to_char(begin_interval_time, 'hh24'), 11,to_char(bytes/1024/1024,'999,999'), null)) "11",
   max(decode(to_char(begin_interval_time, 'hh24'), 12,to_char(bytes/1024/1024,'999,999'), null)) "12",
   max(decode(to_char(begin_interval_time, 'hh24'), 13,to_char(bytes/1024/1024,'999,999'), null)) "13",
   max(decode(to_char(begin_interval_time, 'hh24'), 14,to_char(bytes/1024/1024,'999,999'), null)) "14",
   max(decode(to_char(begin_interval_time, 'hh24'), 15,to_char(bytes/1024/1024,'999,999'), null)) "15",
   max(decode(to_char(begin_interval_time, 'hh24'), 16,to_char(bytes/1024/1024,'999,999'), null)) "16",
   max(decode(to_char(begin_interval_time, 'hh24'), 17,to_char(bytes/1024/1024,'999,999'), null)) "17",
   max(decode(to_char(begin_interval_time, 'hh24'), 18,to_char(bytes/1024/1024,'999,999'), null)) "18",
   max(decode(to_char(begin_interval_time, 'hh24'), 19,to_char(bytes/1024/1024,'999,999'), null)) "19",
   max(decode(to_char(begin_interval_time, 'hh24'), 20,to_char(bytes/1024/1024,'999,999'), null)) "20",
   max(decode(to_char(begin_interval_time, 'hh24'), 21,to_char(bytes/1024/1024,'999,999'), null)) "21",
   max(decode(to_char(begin_interval_time, 'hh24'), 22,to_char(bytes/1024/1024,'999,999'), null)) "22",
   max(decode(to_char(begin_interval_time, 'hh24'), 23,to_char(bytes/1024/1024,'999,999'), null)) "23",
   max(decode(to_char(begin_interval_time, 'hh24'), 24,to_char(bytes/1024/1024,'999,999'), null)) "24"
...
col indx format 9999 head "Array|Row #"
col kghludur format 999 head "Duration"
col inst_id format 999 head "Inst|No"
...
col kghlurcr format 999,999,999,999 head "# Recurrent Chunks"
col kghlutrn format 999,999,999,999 head "# Transient Chunks"
col kghlumxa format 999,999,999,999 head "Alloc Limit"
col kghlumes format 999,999,999,999 Head "Last Size Request|Over Alloc Limit"
col kghlumer format 999,999,999,999 head "# Exceeded|Alloc Limit"
col kghlurcn format 999,999,999,999 head "# scans|Reserved"
col kghlurmi format 999,999,999,999 head "# Misses|Reserved"
col kghlurmz format 999,999,999,999 head "Last Reserved|Size Missed"
col kghlurmx format 999,999,999,999 head "Max Reserved|Size Missed"
col kghlunfu format 999,999,999,999 head "# Times|Looped"
col kghlunfs format 999,999,999,999 head "Last Failed|Size"

select indx, kghluidx, kghludur, kghlurcr,  kghlutrn,  kghlufsh,  kghluops,  kghlunfu,  kghlunfs
from  sys.x$kghlu
where  inst_id = userenv('Instance');

select indx, kghluidx, kghludur, kghlumxa, kghlumes, kghlumer, kghlurcn, kghlurmi, kghlurmz, kghlurmx
from  sys.x$kghlu
where  inst_id = userenv('Instance')
/
