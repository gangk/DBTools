REM   
REM     Runs on 9.2.x and 10.1.x versions
...
select val1, val2, val3, val4, val5, val6
from (select sum(bytes) val1 from gv$sgastat) s1,
    ...
    (select nvl(sum(bytes),0) val5 from gv$sgastat where name='buffer_cache') s6,
    (select nvl(sum(bytes),0) val6 from gv$sgastat where name in ('log_buffer','fixed_sga')) s7;
...
select ' *** If database started recently, this data is not as useful ***',
       ...
       '                                    ',
        ...
        '   Shared Pool Size                 : '
               ||round((&x_sp_size/1024/1024),2)||'M ('
                  ||round((&x_sp_size/&x_sgasize)*100,0)||'%)' sp_size,
       ...
       '                                    ',
        ' *** No guidelines on SQL in Library Cache ***',
       ...
       '# of pinned programmatic construct : '
        ...
        '                                    ',
        '  Estimated Total Library Cache Memory Usage  : '||&x_tot_lc*1.1||'M ('||
               100*(round(((&x_tot_lc*1.1) / (&x_sp_size/1024/1024)),2))||'%)' perc_lc,       
